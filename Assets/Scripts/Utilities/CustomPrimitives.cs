...
public class CustomPrimitives
{
    ...
    private static Mesh Create3DEgg(EggShapes eggShape)
    {
        //height, waistRadius, ratio, vertsPerCircle, circles
        bool backFaces = false;

        float height = eggShape.height;
        float waistRadius = eggShape.waistRadius; //a
        float ratio = eggShape.ratio;  //how many times taller is the top half than the bottom
        float vertsPerCircle = eggShape.vertsPerCircle; //j
        float circleCount = eggShape.circles; //n       minmum 3 for diamond shape, 6 will be blocky, 20?
        ...
        float eggBottomLength = height / (ratio + 1); //b bot
        float eggTopLength = ratio * height / (ratio + 1);
        float pi = Mathf.PI;
        ...
        for (float b = 0; b < rowCount; b++) 
        {
            ...
            float rowBottomRadius = EggRadius(waistRadius, bBotRow, y);
            ...
            float rowTopRadius = EggRadius(waistRadius, bTopRow, y);
    ...
    private static GameObject CreateObject(string _objectToCreate)
    {
        ...
        switch (_objectToCreate)    //creates the mesh from specific methods for each shape
        {
            ...
            case "3D Triangle":
                ...
                break;
            case "Classic Egg":
                var classicEgg = ServiceLocator.Instance.soHolder.classicEgg;
                Debug.Log("classic Egg null?");
                mesh = Create3DEgg(classicEgg);
                break;
            case "Classic Egg2":
                var classicEgg2 = ServiceLocator.Instance.soHolder.classicEgg2;
                Debug.Log("classic Egg null?");
                mesh = Create3DEgg(classicEgg2);
                break;
            case "Slime":
                var slime = ServiceLocator.Instance.soHolder.slime;
                mesh = Create3DEgg(slime);
                break;
            case "Ring":
                mesh = Create3DRing();
                break;
    ...
    [MenuItem("GameObject/3D Object/ClassicEgg", false, 0)]
    public static void CreateClassicEgg()
    {
        CreateObject("Classic Egg");
    }
    [MenuItem("GameObject/3D Object/ClassicEgg2", false, 0)]
    public static void CreateClassicEgg2()
    {
        CreateObject("Classic Egg2");
    }
    [MenuItem("GameObject/3D Object/Slime", false, 0)]
